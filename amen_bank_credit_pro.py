...
def train_tree(df_enc_hash):
    """Entraîne l'Arbre de Décision (classification binaire Good/Bad)."""
    ...
    metrics = {
        ...
        "report":    classification_report(y_te, y_pred, target_names=["Good", "Bad"]),
        ...
        <div style="background:white;border-radius:20px;overflow:hidden;
          ...
          <div style="padding:1.5rem 2rem">
            ...
            <div style="background:{VERT_BG};border-radius:10px;padding:.7rem 1rem;
              ...
              🌳 <b>Decision Tree</b> — Classification &nbsp;|&nbsp; ⚡ <b>XGBoost</b> — Prédiction % Risque
            </div>
          </div>
        </div>
        """, unsafe_allow_html=True)
...
def page_dashboard(df):
    ...
    kpi(k5, f"{good['Credit amount'].mean():,.0f}","Montant Moy (Good)",   "✅", "kpi-success")
    kpi(k6, f"{bad['Credit amount'].mean():,.0f}", "Montant Moy (Bad)",    "⚠️", "kpi-danger")
    ...
    with col1:
        section("Répartition Good / Bad")
        rc = df["Risk"].value_counts()
        fig = go.Figure(go.Pie(
            labels=["✅ Bon Client", "❌ Mauvais Client"],
    ...
    with col3b:
        ...
        fig3.add_trace(go.Histogram(x=bad["Age"],  name="❌ Mauvais",     nbinsx=25, marker_color=ROUGE,  opacity=0.75))
    ...
    if "% Risque" in df.columns:
        ...
        with col5a:
            section("🔥 Distribution du Score % Risque — Good vs Bad")
            fig5 = go.Figure()
            for risk, clr in [("good", VERT_C), ("bad", ROUGE)]:
                fig5.add_trace(go.Histogram(
                    x=df[df["Risk"] == risk]["% Risque"],
                    name=risk, nbinsx=40, marker_color=clr, opacity=0.7,
                ))
        ...
        with col5b:
            section("📋 Tableau Statistique Comparatif Good vs Bad")
            stats_rows = []
            for col_n, label in [
...
def page_eda(df):
    ...
    with tab1:
        ...
        for i, col in enumerate(NUM):
            ...
            for risk, clr in [("good", VERT_C), ("bad", ROUGE)]:
                fig.add_trace(go.Histogram(
                    x=df[df["Risk"] == risk][col], name=risk,
        ...
        with col1:
            fig_b = px.box(df, x="Risk", y="Credit amount", color="Risk",
                           color_discrete_map={"good": VERT_C, "bad": ROUGE}, points="outliers")
            plotly_layout(fig_b, height=340)
            st.plotly_chart(fig_b, use_container_width=True)
        with col2:
            fig_v = px.violin(df, x="Risk", y="Duration", color="Risk",
    ...
    with tab4:
        ...
        for col in NUM:
            rows.append({
                "Variable":  col,
                "Moy GOOD":  f"{good2[col].mean():.2f}",
                "Méd GOOD":  f"{good2[col].median():.2f}",
                "Éc.T GOOD": f"{good2[col].std():.2f}",
                "Moy BAD":   f"{bad2[col].mean():.2f}",
                "Méd BAD":   f"{bad2[col].median():.2f}",
...
def page_tree(df_enc, clf, tree_metrics, splits):
    render_header(
        ...
        "Classification binaire Good / Bad · max_depth=5 · class_weight=balanced",
    )

    # KPIs
    c1, c2, c3, c4 = st.columns(4)
    kpi(c1, f"{tree_metrics['accuracy']}%", "Accuracy",    "🎯", "kpi-success")
    kpi(c2, f"{tree_metrics['f1']}%",       "F1-Score",    "📊", "kpi-or")
    kpi(c3, f"{tree_metrics['precision']}%","Précision",   "🎯", "kpi-blue")
    kpi(c4, f"{tree_metrics['cv']}%",       "CV-Accuracy", "🔄", "")
    ...
    with col1:
        ...
        fig_cm = go.Figure(go.Heatmap(
            z=cm, x=["Prédit Good", "Prédit Bad"], y=["Réel Good", "Réel Bad"],
    ...
    with st.expander("🌳 Visualisation Graphique de l'Arbre"):
        ...
        plot_tree(
            clf, feature_names=FEATURES, class_names=["Good", "Bad"],
...
def page_xgb(df_enc, xgb_model, scaler, xgb_met):
    ...
    c1, c2, c3, c4 = st.columns(4)
    kpi(c1, f"{xgb_met['r2']}%",   "R² Score", "📈", "kpi-success")
    kpi(c2, f"{xgb_met['mae']}",   "MAE",      "📉", "kpi-or")
    kpi(c3, f"{xgb_met['rmse']}",  "RMSE",     "📊", "kpi-blue")
    kpi(c4, f"{xgb_met['mse']}",   "MSE",      "🔢", "")
        ...
        result_data = [
            [
                ...
                Paragraph(f"<b><font size='14'>Confiance Arbre</font></b><br/>"
                          ...
                          f"<font size='9'>P(Good)={tree_proba[0]*100:.1f}% | P(Bad)={tree_proba[1]*100:.1f}%</font>",
...
def page_prediction(df, df_raw, df_enc, encoders, clf, xgb_model, scaler):
    ...
    st.markdown(f"""
    <div class="info-bar">
      🌳 <b>Arbre de Décision</b> — Classification Good/Bad &nbsp;|&nbsp;
...
def page_data(df):
    ...
    with tab1:
        ...
        with col_f1:
            filter_risk = st.selectbox("Filtrer par risque", ["Tous", "good", "bad"])
        with col_f2:
            filter_sex  = st.selectbox("Filtrer par sexe", ["Tous"] + sorted(df["Sex"].dropna().unique().tolist()))
        with col_f3:
            filter_purp = st.selectbox("Filtrer par objet", ["Tous"] + sorted(df["Purpose"].dropna().unique().tolist()))

        df_view = df.copy()
        if filter_risk != "Tous": df_view = df_view[df_view["Risk"]    == filter_risk]
        if filter_sex  != "Tous": df_view = df_view[df_view["Sex"]     == filter_sex]
        if filter_purp != "Tous": df_view = df_view[df_view["Purpose"] == filter_purp]
        ...
        for idx, row in hist_f.iterrows():
            ...
            for c in cols_exist:
                ...
                if c == "Classification":
                    ...
                    icon = "👨" if str(val).lower() in ["male", "homme"] else "👩"
                    cell_content = f"{icon} {val}"
